...
int DxLib_Init(){
    ...
    if((Screen.window = SDL_CreateWindow(Screen.title.c_str(), SDL_WINDOWPOS_CENTERED,  SDL_WINDOWPOS_CENTERED, Screen.width, Screen.height, Screen.mode)) == NULL)
        return -1;
...
int SetGraphMode(int width, int height, int bit){
    ...
    if(Screen.window != NULL && Screen.renderer != NULL){
        ...
        if((Screen.window = SDL_CreateWindow("DxLib", SDL_WINDOWPOS_CENTERED,  SDL_WINDOWPOS_CENTERED, Screen.width, Screen.height, Screen.mode)) == NULL)
            return -1;
...
int DrawTriangleAA(float x1, float y1, float x2, float y2, float x3, float y3, unsigned int color, int fill, float thick){
    unsigned char *c = (unsigned char*)&color;
...
int DrawRoundRectAA(float x1, float y1, float x2, float y2, float rx, float ry, int posnum, unsigned int color, int fill, float thick){
    unsigned char *c = (unsigned char*)&color;
    double dx,dy;
    bool need = true;
...
int DrawQuadrangleAA(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, unsigned int color, int fill, float thick){
    unsigned char *c = (unsigned char*)&color;
...
int DrawRectRotaGraph(int x, int y, int tx, int ty, int w, int h, double Ex, double angle, int graph, int trans, int turnX, int turnY){
    map<int, GRAPH_SAVER*>::iterator target;
    double gw, gh;
...
int DrawRectRotaGraphF(float x, float y, int tx, int ty, int w, int h, double Ex, double angle, int graph, int trans, int turnX, int turnY){
    double gw, gh;
    map<int, GRAPH_SAVER*>::iterator target;
